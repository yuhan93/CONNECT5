...
<body>
	...
	<div class="search">
		<form method="get"
			action="${pageContext.request.contextPath}/_coach/QnA_SE.do">
			<ul id="key">
					<li>
	...
	<div class="col-md-10 content">
				...
				<tbody>
					<c:choose>
						...
						<c:otherwise>
							<%-- 조회 결과에 따른 반복 처리 --%>
							<c:set var="num"
								value="${pageData.totalCount - ((pageData.nowPage - 1) * pageData.listCount)}" />
		...
		<c:choose>
			...
			<c:when test="${pageData.prevPage > 0}">
				<%-- 이동할 URL 생성 --%>
				<c:url value="/_coach/QnA_SE.do" var="prevPageUrl">
		...
		<c:forEach var="i" begin="${pageData.startPage}"
			...
			<%-- 이동할 URL 생성 --%>
			<c:url value="/_coach/QnA_SE.do" var="pageUrl">
		...
		<c:choose>
			...
			<c:when test="${pageData.nextPage > 0}">
				<%-- 이동할 URL 생성 --%>
				<c:url value="/_coach/QnA_SE.do" var="nextPageUrl">
		...
		<button class="btn btn-default"
			onclick="location.href = '${pageContext.request.contextPath}/_coach/QnAWrite_SE.do'">글쓰기</button>
