...
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/assets/css/YB/Admin/MngBoard_WeddingStory_YB.css">
		...
		<div class="col-md-10 text_box">
				...
				<tbody>
					<c:choose>
						<c:when test="${output == null || fn:length(output) == 0}">
							<tr>
								<td colspan="9" align="center">성혼스토리 게시물 조회결과가 없습니다.</td>
							</tr>
						</c:when>
						...
						<c:otherwise>
							...
							<c:forEach var="item" items="${output}" varStatus="status">
								...
								<%-- 상세페이지로 이동하기 위한 URL --%>
								<c:url value="/_admin/admin_MngBoard_WeddingStoryRead_YB.do" var="viewUrl">
								...
								<tr>
									...
									<td><button type="button" class="label label-warning" onclick="location.href = '${pageContext.request.contextPath}/_admin/admin_MngBoard_WeddingStory_DeleteOk.do?BoardId=${BoardId}'">삭제</button></td>
								</tr>
			...
			<c:choose>
				...
				<c:when test="${pageData.prevPage > 0}">
					<%-- 이동할 URL 생성 --%>
					<c:url value="/_admin/admin_MngBoard_WeddingStory_YB.do" var="prevPageUrl">
			...
			<c:forEach var="i" begin="${pageData.startPage}"
				...
				<%-- 이동할 URL 생성 --%>
				<c:url value="/_admin/admin_MngBoard_WeddingStory_YB.do" var="pageUrl">
			...
			<c:choose>
				...
				<c:when test="${pageData.nextPage > 0}">
					<%-- 이동할 URL 생성 --%>
					<c:url value="/_admin/admin_MngBoard_WeddingStory_YB.do" var="nextPageUrl">
